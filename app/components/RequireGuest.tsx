...
export default function RequireGuest({ children }: { children: React.ReactNode }) {
  const { guest, loginWithCode, loading } = useGuest();
  ...
  // 1. Dyskretne ładowanie początkowe
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <p className="animate-pulse font-serif tracking-widest text-lg opacity-60">Sprawdzanie uprawnień...</p>
      </div>
    );
  }

  // 2. Gość JEST zalogowany -> pokazujemy zawartość
  if (guest) {
    return <>{children}</>;
  }

  // 3. Gość NIE JEST zalogowany -> pokazujemy widget autoryzacji w miejscu ukrytej treści
  return (
    ...
    <AnimatePresence mode="wait">
      <motion.section
          ...
          // ZMIANA: Zmieniono items-center na items-start, żeby okienko trzymało się góry pod navbarem
          className="w-full flex items-start justify-center overflow-hidden min-h-screen"
        >
          <div className="w-full flex justify-center pb-2 relative z-10 pt-8 md:pt-4"> 
            <motion.div
            ...
            >
              ...
              <div className="relative z-10 w-full flex flex-col items-center text-center -mt-3 md:-mt-4">
                ...
                <p className="mb-8 text-[#FDF9EC]/90 font-medium px-2 text-sm md:text-base">
                  Wygląda na to, że jakieś nieprzychylne siły Cię wylogowały! Musisz raz jeszcze podać swój kod, aby zobaczyć tę zawartość.
                </p>
